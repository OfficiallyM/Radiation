...
namespace Radiation.Utilities
{
	...
	// Source: https://github.com/MinaPecheux/UnityTutorials-NoisesProceduralGeneration/blob/master/Assets/Scripts/Noises/WorleyNoise.cs
	internal static class Noise
	{
		...
		private const float _jitter = 1f; // Less gives more regular pattern
		...
		public static float GetNoiseMap(float x, float y, float scale = 1f)
		{
			x *= scale;
			y *= scale;
		...
		private static float _Mod(float x, float y)
		{
			return x - y * Mathf.Floor(x / y);
		}

		private static float _Frac(float v)
		{
			return v - Mathf.Floor(v);
		}

		private static float _Distance2(float p1x, float p1y, float p2x, float p2y)
		{
		...
		private static int _PermAt(int i)
		{
			return _Perm[i & PERM_WRAP] & PERM_MAX;
		}

		private static int _PermAt(int i, int j)
		{
